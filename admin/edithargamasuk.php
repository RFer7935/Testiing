...
<body>
            ...
            <div class="container-fluid px-4">
                <div class="row">
                    <div class="col-12">
                        ...
                        <!-- Pricing Section -->
                        <div id="pricing-section" class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_pricing'])) {
                                    $pricing_text = $_POST['pricing_text'];
                                    $sql = "UPDATE pricing_section SET text = '$pricing_text' WHERE id = 1";

                                    if (mysqli_query($conn, $sql)) {
                                        echo "<div class='alert alert-success'>Data berhasil diupdate!</div>";
                                        echo "<script>
                                                setTimeout(function() {
                                                    window.location.href = 'edithargamasuk.php';
                                                }, 1000);
                                              </script>";
                                    } else {
                                ...
                                $pricing_data = mysqli_fetch_assoc($result);
                                ?>
                                <form method="POST">
                                    <div class="mb-3">
                                        <label for="pricing_text" class="form-label">Isi Text:</label>
                                        <textarea class="form-control" id="pricing_text" name="pricing_text" rows="3"
                                            required disabled><?php echo $pricing_data['text']; ?></textarea>
                                    </div>
                                    <button type="button" class="btn btn-primary" id="edit_button">Edit</button>
                                    <button type="submit" class="btn btn-success" name="update_pricing" id="save_button"
    ...
    <script>
        document.getElementById('edit_button').addEventListener('click', function () {
            document.getElementById('pricing_text').disabled = false;
